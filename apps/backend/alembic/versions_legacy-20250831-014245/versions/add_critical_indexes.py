...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_products_stock", "products", ["stock_on_hand", "stock_reserved"]
    )
    ...
    # Order items indexes
    op.create_index(
        "ix_order_items_product_order", "order_items", ["product_id", "order_id"]
    )
    ...
    op.create_index(
        "ix_suppliers_file_path_unique", "suppliers", ["file_path"], unique=True
    )
