...
def upgrade() -> None:
    ...
    op.create_check_constraint(
        "ck_products_stock_on_hand_nonneg", "products", "stock_on_hand >= 0"
    )
    op.create_check_constraint(
        "ck_products_stock_reserved_nonneg", "products", "stock_reserved >= 0"
    )
    op.create_check_constraint(
        "ck_stock_movements_qty_change_not_zero", "stock_movements", "qty_change != 0"
    )
...
def downgrade() -> None:
    ...
    op.drop_constraint(
        "ck_stock_movements_qty_change_not_zero", "stock_movements", type_="check"
    )
