...
@router.post("/", response_model=ProductOut, dependencies=[Depends(require_any_role("admin","manager"))])
...
@router.put("/{pid}", response_model=ProductOut, dependencies=[Depends(require_any_role("admin","manager"))])
...
@router.delete("/{pid}", dependencies=[Depends(require_any_role("admin"))])
