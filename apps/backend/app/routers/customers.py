...
def list_customers(q: str | None = Query(default=None), db: Session = Depends(get_db), user=Depends(require_role)):
...
@router.put("/{cid}", response_model=CustomerOut, dependencies=[Depends(require_role("admin"))])
...
@router.delete("/{cid}", dependencies=[Depends(require_role("admin"))])
